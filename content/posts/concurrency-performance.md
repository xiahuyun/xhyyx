+++
title = '并发性能优化复盘'
date = 2024-06-18T11:40:38+08:00
categories = ["go"]
...
这个接口的大致情况是这样：
1. 服务治理系统调用 `/environments/list` 获取平台的所有环境，假设每个环境对应一个 kubernetes 集群。我们的平台大概有 30 个环境。
2. 服务治理系统调用 `/environments/:name/resouce` 获取对应环境下资源。
3. 后端 `environments/:name/resource`  会并发起多个协程访问代理系统的 api，获取到结果并处理后返回给调用方。

![访问示意图.png](/images/concurrency-performance-arch.png)

服务治理系统的使用场景是：
1. 每天晚上调用 `environments/list` 获取平台的所有环境，然后遍历调用 `/environments/:name/resource` 获取所有环境下的资源。
2. 服务治理系统只调用一次，不会重复/并发调用。
...
1. 为了加速 api 的响应，后端创建 20 个协程并发访问代理系统接口获取信息并处理。
2. 并发协程会创建自己的代理系统客户端，然后拿着这个客户端请求代理系统 api。
3. 并发协程的代理系统客户端，有一部分配置是公用的，一部分是协程独有的。
...
![协程示意图.png](/images/concurrency-performance-goroutines.png)
...
这样也会有问题：
1. 每次调用都需要预热缓存，实际只需要预热一次就够了。
2. 每次调用创建 100 个协程，go 运行时需要管理这 100 个协程的创建/销毁/复用/调度，增加了运行时的负担。
